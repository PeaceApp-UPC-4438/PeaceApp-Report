...
| TS11 | Colorear zonas en el mapa según reportes | Como desarrollador, quiero que el mapa coloree automáticamente las zonas en función de la cantidad de reportes para mostrar visualmente las áreas más peligrosas o seguras. | Escenario 1: Colorear una zona con múltiples reportes<br>Dado que el endpoint /api/v1/map/regions está disponible<br>Cuando se envía un POST request con los datos de ubicación y cantidad de reportes<br>Entonces se recibe un response con un status 201<br>Y el mapa se actualiza coloreando la zona con un color que representa la peligrosidad basada en la cantidad de reportes.<br><br>Escenario 2: Colorear una zona sin reportes<br>Dado que el endpoint /api/v1/map/regions está disponible<br>Cuando se envía un POST request para una zona sin reportes<br>Entonces se recibe un response con un status 200<br>Y el color de la zona se mantiene como "segura" en el mapa. | No corresponde |
| TS12 | Obtener el estado de una zona en el mapa | Como desarrollador, quiero que los usuarios puedan obtener el estado actual de una zona para verificar si es segura o peligrosa según la cantidad de reportes. | Escenario 1: Obtener estado de una zona con reportes<br>Dado que el endpoint /api/v1/map/regions/{regionId} está disponible<br>Cuando se envía un GET request con el ID de una región<br>Entonces se recibe un response con un status 200<br>Y el estado actual de la zona (segura, moderada, peligrosa) es devuelto en el body del response.<br><br>Escenario 2: Intentar obtener el estado de una zona inexistente<br>Dado que el endpoint /api/v1/map/regions/{regionId} está disponible<br>Cuando se envía un GET request con un ID de región inexistente<br>Entonces se recibe un response con un status 404<br>Y un mensaje en el body del response dice "Zona no encontrada." | No corresponde |
| TS13 | Enviar notificación a los usuarios mediante RESTful API | Como desarrollador, quiero que los usuarios puedan recibir notificaciones en la plataforma para que se encuentren informados en tiempo real. | Escenario 1: Enviar notificación válida<br>Dado que el endpoint /api/v1/notifications está disponible<br>Cuando se envía un POST request con los datos del evento importante<br>Entonces se recibe un response con un status 201<br>Y los usuarios reciben la notificación en la plataforma con los detalles del evento.<br><br>Escenario 2: Intentar enviar notificación sin datos válidos<br>Dado que el endpoint /api/v1/notifications está disponible<br>Cuando se envía un POST request sin los datos necesarios<br>Entonces se recibe un response con un status 400<br>Y un mensaje en el body del response dice "Datos insuficientes para generar la notificación." | No corresponde |
| TS14 | Obtener lista de notificaciones enviadas a los usuarios mediante RESTful API | Como desarrollador, quiero que los usuarios puedan revisar las notificaciones importantes que han recibido previamente para consultar eventos pasados. | Escenario 1: Obtener lista de notificaciones<br>Dado que el endpoint /api/v1/notifications está disponible<br>Cuando se envía un GET request<br>Entonces se recibe un response con un status 200<br>Y una lista de las notificaciones recibidas es devuelta en el body del response.<br><br>Escenario 2: No hay notificaciones disponibles<br>Dado que el endpoint /api/v1/notifications está disponible<br>Cuando se envía un GET request<br>Entonces se recibe un response con un status 200<br>Y un mensaje en el body dice "No hay notificaciones disponibles." | No corresponde |
| TS15 | Eliminar notificación mediante RESTful API | Como desarrollador, quiero que los usuarios puedan eliminar las notificaciones que ya no son relevantes para mantener su bandeja de notificaciones organizada. | Escenario 1: Eliminar notificación existente<br>Dado que el endpoint /api/v1/notifications/{id} está disponible<br>Cuando se envía un DELETE request con el ID de la notificación<br>Entonces se recibe un response con un status 204<br>Y la notificación es eliminada del sistema.<br><br>Escenario 2: Intentar eliminar notificación inexistente<br>Dado que el endpoint /api/v1/notifications/{id} está disponible<br>Cuando se envía un DELETE request con un ID inexistente<br>Entonces se recibe un response con un status 404<br>Y un mensaje en el body dice "Notificación no encontrada." | No corresponde |
...
| 19     | **US19**                            | Recuperar Contraseña                                                         | Como usuario, quiero poder personalizar mi perfil, para reflejar mi identidad en la aplicación.                                                                                    | 2                            |
| 20     | **TS05**                            | Crear reporte de incidente mediante RESTful API                              | Como desarrollador, quiero que los usuarios puedan crear reportes de incidentes para compartir información sobre zonas peligrosas.                                                 | 5                            |
| 21     | **TS08**                            | Enviar alerta de emergencia mediante RESTful API                             | Como desarrollador, quiero que los usuarios puedan enviar alertas de emergencia para que reciban ayuda inmediata.                                                                  | 8                            |
| 22     | **TS10**                            | Obtener reporte por ID mediante RESTful API                                  | Como desarrollador, quiero que los usuarios puedan obtener los detalles de un solo reporte para consultar información específica sobre un incidente.                               | 3                            |
| 23     | **TS06**                            | Obtener lista de reportes mediante RESTful API                               | Como desarrollador, quiero que los usuarios puedan obtener una lista de reportes para ver incidentes recientes en su área.                                                         | 3                            |
| 24     | **TS13**                            | Enviar notificación a los usuarios mediante RESTful API                      | Como desarrollador, quiero que los usuarios puedan recibir notificaciones en la plataforma para que se encuentren informados en tiempo real.                                       | 5                            |
| 25     | **TS14**                            | Obtener lista de notificaciones enviadas a los usuarios mediante RESTful API | Como desarrollador, quiero que los usuarios puedan revisar las notificaciones importantes que han recibido previamente para consultar eventos pasados.                             | 3                            |
| 26     | **TS11**                            | Colorear zonas en el mapa según reportes                                     | Como desarrollador, quiero que el mapa coloree automáticamente las zonas en función de la cantidad de reportes para mostrar visualmente las áreas más peligrosas o seguras.        | 8                            |
| 27     | **TS12**                            | Obtener el estado de una zona en el mapa                                     | Como desarrollador, quiero que los usuarios puedan obtener el estado actual de una zona para verificar si es segura o peligrosa según la cantidad de reportes.                     | 5                            |
| 28     | **TS07**                            | Eliminar reporte de incidente mediante RESTful API                           | Como desarrollador, quiero permitir que los usuarios eliminen reportes que hayan creado previamente para que puedan gestionar la información que han compartido en la plataforma.  | 3                            |
| 29     | **TS15**                            | Eliminar notificación mediante RESTful API                                   | Como desarrollador, quiero que los usuarios puedan eliminar las notificaciones que ya no son relevantes para mantener su bandeja de notificaciones organizada.                     | 2                            |
| 30     | **TS01**                            | Autenticación JWT mediante RESTful API                                       | Como desarrollador, quiero autenticar a los usuarios a través de un token JWT para que puedan acceder a la plataforma de manera segura.                                            | 8                            |
| 31     | **TS02**                            | Crear nuevo usuario mediante RESTful API                                     | Como desarrollador, quiero permitir la creación de nuevos usuarios para que puedan acceder al sistema.                                                                             | 5                            |
| 32     | **TS03**                            | Editar perfil de usuario mediante RESTful API                                | Como desarrollador, quiero que los usuarios puedan actualizar su información personal para mantener sus perfiles al día.                                                           | 3                            |
| 33     | **TS09**                            | Obtener lista de alertas enviadas mediante RESTful API                       | Como desarrollador, quiero que los usuarios puedan obtener una lista de las alertas que han enviado previamente para que puedan revisar su historial de alertas.                   | 3                            |
| 34     | **TS04**                            | Eliminar cuenta de usuario mediante RESTful API                              | Como desarrollador, quiero permitir que los usuarios eliminen sus cuentas para que puedan gestionar su presencia en la plataforma.                                                 | 5                            |
